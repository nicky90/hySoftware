...
*/
void MD5Transform(unsigned buf[4], unsigned const in[16])
{
...
void Md5Buffer(unsigned char digest[MD5_HASHBYTES], unsigned char const *buf, unsigned len)
{
	MD5_CTX ctx;
...
int Md5File(unsigned char digest[MD5_HASHBYTES], const char* path)
{
	int     fd = -1, ret = 0;
	ssize_t nread;
	MD5_CTX ctx;
	uint8_t data[FILE_BUFFER_LEN];

	fd = open(path, O_RDONLY);
	if(-1 == fd) return -1;
	MD5Init(&ctx);
	while(1){
		nread = read(fd, data, FILE_BUFFER_LEN);
		if(nread < 0){
			ret = -1;
			break;
		}else if(0 == nread) break;
		MD5Update(&ctx, data, nread);
	}
	MD5Final(digest, &ctx);
	close(fd);
...
int Md5Cmp(unsigned char da[MD5_HASHBYTES], unsigned char db[MD5_HASHBYTES])
{
	int i;
	for(i = 0; i < MD5_HASHBYTES; i++)
